...
class ThreadBuffer:
    ...
    def __iter__(self):
        ...
        try:
            ...
            self.stop()  # ensure thread compilation
...
def buffer_iterator(src, buffer_size: int = 1, timeout: float = 0.01, repeats: int = 1):
    ...
    Args:
        ...
        buffer_size: Number af items to buffer from the source
        timeout: Time to wait for an item from the buffer, or to wait while the buffer is full when adding items
...
class _ProcessThread(Thread):
    ...
    def run(self):
        try:
            ...
            torch.utils.data._utils.worker._worker_info = None  # clean up global data used for process


class _ProcessorQueue(Queue):
...
class _ProcessorThreadContext(SpawnContext):
    _name = 'processthread'
    ...
    Queue = _ProcessorQueue  # type: ignore
...
class ThreadDataLoader(DataLoader):
    ...
    iterate over data from the loader as expected however the data is genearted on a separate thread. Use this class
    ...
    generation process more time to produce a result. This duplication is done by simple yielding the same object many
    ...
    The `use_thread_workers` will cause workers to be created as threads rather than proceses although everything else
    ...
    and random transforms are not thread-safe and can't owrk as expected with `thread workers`, need to check all the
    ...
    def __init__(
        ...
        **kwargs
        ...
        if use_thread_workers and kwargs.get('number_workers', 0) > 0:
            kwargs['multiprocessing_context'] = _ProcessorThreadContext()
